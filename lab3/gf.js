...
function setup() {
    ...
    platforms.push({ x: 300, y: 450, width: 150, height: 10 }); // Первая платформа
    platforms.push({ x: 480, y: 380, width: 150, height: 10 }); // Вторая платформа
    platforms.push({ x: 300, y: 310, width: 150, height: 10 }); // Вторая платформа
...
function drawGame() {
    ...
    image(muteIcon, (width - iconSize) / 2, 10, iconSize, iconSize); // Позиция и размер 
    ...
    if (drawButton(680, 60, 100, 35, "Main menu", resetGame)) {
        // resetGame вызывается при нажатии на кнопку
    }
}
...
function drawback() {
    ...
    drawCanyon(180, 540);
    let iconSize = 80; // Размер изображения
    image(muteIcon, (width - iconSize) / 2, 10, 40, 40); // Позиция и размер 
}
