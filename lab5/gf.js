...
let isFalling = false; // состояние падения
let fallSpeed = 0; // скорость падения
...
function draw() {
    // Вызываем функции из массива scenery
...
function drawCanyon(x, y) {
    // Чёрная дыра (открытая часть люка)
    fill(0); // Чёрный цвет
    ellipse(x, y, 80, 30); // Основное дно (чёрная дыра)
    quad(140, 540, 220, 550, 220, 600, 140, 600);

    // Обводка люка
    stroke(150); // Серый цвет для обводки
    strokeWeight(5);
    noFill();
    ellipse(x, y, 85, 35); // Ободок люка

    // Крышка рядом с люком
    fill(80); // Цвет крышки (темно-серый)
    noStroke();
    rect(x + 50, y - 10, 70, 10, 5); // Прямоугольная крышка
    fill(150); // Более светлый серый
    ellipse(x + 50, y - 10, 20, 10); // Декоративная часть на крышке
}
...
function fallCrab() {
    fallSpeed += gravity; // Увеличиваем скорость падения
    crabY += fallSpeed; // Двигаем краба вниз

    if (crabY >= height) { // Если краб упал ниже экрана
        crabY = 500; // Возвращаем краба на начальную позицию
        crabX = 400;
        isFalling = false; // Завершаем падение
    }

    drawCrabJump(crabX, crabY); // Рисуем краба во время падения
}
